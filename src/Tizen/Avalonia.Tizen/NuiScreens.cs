...
internal class NuiScreens : ScreensBaseImpl<int, SingleTizenScreen>
{
    ...
    protected override void RefreshScreen(SingleTizenScreen screen) => screen.Refresh();
}

internal class SingleTizenScreen(int index) : Screen(new PlatformHandle(new IntPtr(index), nameof(SingleTizenScreen)))
{
    public void Refresh()
    {
        IsPrimary = index == 1;
